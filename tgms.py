...
class NeuralTextureTrainer(BaseTrainer):
    def __init__(self, device, img_dir, uv_dir, ref_dir, name, length, batch_size=32):
    ...
    def build_network(self):
        ...
        self.model =nn.DataParallel(self.model,device_ids=[0,1],output_device='cuda:0').cuda()
        # self.model = self.model.to(self.device)
        self.model.train()

    def build_optimizer(self):
        self.lr = 0.001
        for m in self.model.module.texture.parameters():
            m.requires_grad = True

        self.optimizer = Adam([
            {'params': self.model.module.texture.layer1, 'lr': self.lr}
        ])
    ...
    def train(self):
        ...
        for epoch_idx in range(1, 1 + epoch):
            ...
            save_tex(self.model.module, self.coarse_texture_path, self.name)
        pass
...
class NeuralRenderingTrainer(BaseTrainer):
    ...
    def build_optimizer(self):
        self.lr = 0.001
        for m in self.model.module.renderer.parameters():
            m.requires_grad = True
        for m in self.model.module.texture.parameters():
            m.requires_grad = True  # False

        for idx in range(3):
            for m in self.model.module.texture.textures[idx].parameters():
                m.requires_grad = False

        self.optimizer = Adam([
            {'params': self.model.module.texture.layer1, 'lr': self.lr},
            {'params': self.model.module.renderer.parameters(), 'lr': 0.1 * self.lr},
        ])
    ...
    def load_weights(self, mode='train'):
        if mode == 'train':
            ...
            self.model.module.texture.load_state_dict(pretrained_dict)

            if os.path.exists(self.pretrained_checkpoint_path):
                self.model.module.renderer.load_state_dict(torch.load(self.pretrained_checkpoint_path))
        elif mode == 'test':
            self.model.module.texture.load_state_dict(torch.load(f'{self.save_checkpoint_path}/texture_{self.name}.pt'))
            self.model.module.renderer.load_state_dict(torch.load(f'{self.save_checkpoint_path}/renderer_{self.name}.pt'))
    ...
    def train(self):
        ...
        for epoch_idx in range(1, 1 + epoch):
            ...
            for step, samples in enumerate(self.dataloader):
                ...
                images, uv_maps, ref_images = images.to(device), uv_maps.to(device), ref_images.to(device)
            ...
            torch.save(self.model.module.texture.state_dict(),  f'{self.save_checkpoint_path}/texture_{self.name}.pt')
            torch.save(self.model.module.renderer.state_dict(), f'{self.save_checkpoint_path}/renderer_{self.name}.pt')
...
class TexOptimizer(BaseTrainer):
    ...
    def build_optimizer(self):
        ...
        self.optimizer = Adam([
            {'params': self.model.module.texture.layer1, 'lr': self.lr},  # * 0.5 args.lr
        ])
